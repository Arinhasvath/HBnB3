...
class UserList(Resource):
    ...
    def get(self):
        """List all users"""
        try:
            return facade.get_all_users()  # Assurez-vous que cette méthode existe
        except Exception as e:
            api.abort(500, str(e))

    @api.doc('create_user')
    @api.expect(api.model('UserInput', {
...
class User(Resource):
    ...
    def get(self, user_id):
        """Fetch a user by ID"""
        try:
            user = facade.get_user(user_id)  # Assurez-vous que cette méthode existe
            if user is None:
